...
export default function ExtraClassWeekSelectorPopup({ extraClassId, onClose }: Props) {
  ...
  const fetchWeeks = async () => {
    const res = await axiosInstance.get(`${API.WEEKLY_EXTRA_RECORDS}/extraClass/${extraClassId}/week`, {
  ...
  const handleMove = () => {
    if (selected !== null)
      navigate(`/extra-class/${extraClassId}/week/${selected}`);
  };
  ...
  const handleConfirmCreate = () => {
    ...
    navigate(`/extra-class/${extraClassId}/week/${newWeekNo}`);
  };
  ...
  return (
    <Overlay onClick={onClose}>
      <Popup onClick={(e : React.MouseEvent<HTMLDivElement>) => e.stopPropagation()}>
        ...
        <WeekList>
          {weekList.length === 0 ? (
            <WeekItem selected={false} as="div" style={{ textAlign: "center", color: "#999" }}>
              📭 등록된 주차가 없습니다.
            </WeekItem>
          ) : (
            weekList.map((week) => (
